...
Within every serialized node, child element tags and `xmlns*` attributes are emitted in **first-seen input order** — the order they appeared in the source XML — not alphabetical order. Alphabetical order enters only among keys whose relative position none of the three inputs determines; every tag whose position any side determines keeps its first-seen position regardless. This is established at two points in the pipeline:

- **Merge-time**: `mergePropertyOrder` (`src/merger/mergePropertyOrder.ts`) merges the three sides' key *sequences*, not a flat set. It derives precedence edges from each side's consecutive key pairs — ancestor, local and other alike — takes a Kahn topological sort over the union of the three key sets, and, whenever several keys are simultaneously ready, breaks the tie by a **lexicographic** ordering of the key names (a bare `Array.prototype.sort()`, never `localeCompare`). The same lexicographic rank repairs the case where two sides insert the same keys in incompatible relative order (a cycle in the precedence graph): the lowest-ranked unemitted key is emitted first. A fast path — `sameSequence(local, other) && isSubsequence(ancestor, local)` — returns `local`'s key sequence directly whenever both sides already agree on an order the ancestor is consistent with; that condition makes the topological order unique, so the result there does not depend on the tie-break rank at all.
...
Four further properties of the tie-break are worth stating explicitly:
