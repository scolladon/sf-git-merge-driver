...
### Root xmlns Merge Is Last-Write-Wins, Not Three-Way

The root element's `xmlns*` attributes are extracted into a separate `namespaces` bucket by the parser adapter and never enter the JSON `content` tree (see "Compact JSON Intermediate Representation" above), so they never go through `MergeOrchestrator`/`ScenarioStrategy` at all. `XmlMerger.mergeNamespaces` combines the three sides with a plain `Object.assign({}, ancestor, local, other)`: whichever side declares a given prefix last always wins the key, including when only `local` changed it and `other` is untouched. This is deliberate, not an oversight — a genuine three-way divergence on an attribute value has no clean way to carry zdiff3 markers without producing invalid XML (`xmlns="<<<<<<< ours..."`), so the driver picks the same fallback used by many merge tools for un-representable conflicts rather than raising one. Pinned by the `given namespace merge across three sides` and `given the ancestor stream is delayed behind ours and theirs` cases in `test/unit/merger/XmlMerger.streaming.test.ts`. In practice this is rarely observable: the metadata types this driver targets all declare the same fixed `http://soap.sforce.com/2006/04/metadata` namespace, which is not something users hand-edit.

### Repeated-Scalar Set Semantics Depend on Cardinality, Not Schema

`TextArrayMergeNode` — the "union of adds, ancestor-relative deletes, never conflicts" strategy used for `package.xml` `<members>` and similar repeated scalar tags — is only selected when `MergeNodeFactory.createNode` sees an actual array (2+ occurrences) of strings on at least one of the three sides (`isStringArray` in `MergeNodeFactory.ts`). A repeatable tag that happens to have exactly one occurrence on all three sides parses to a bare scalar instead, and is compared with the strict, always-can-conflict `TextMergeNode` rules. Concretely: renaming the sole `<members>` entry of a `<types>` block while the other side also renames it differently produces a real conflict at cardinality 1, but silently keeps *both* renamed values with no conflict at cardinality 2+ — same edit, different outcome, driven entirely by how many elements happened to already be there. Not fixed here: recovering schema-driven (rather than cardinality-driven) routing would need a table of "these tags are always set-semantics" similar to `METADATA_KEY_EXTRACTORS`, and it is not obvious that the cardinality-1 behavior is wrong often enough in real `package.xml` usage (which almost always has 2+ `<members>` per `<types>` block) to justify it.
